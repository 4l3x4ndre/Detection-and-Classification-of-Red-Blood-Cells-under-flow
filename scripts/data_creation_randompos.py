...
#plt.imshow(add_walls_gray(generate_noise_background(200, 200)[0]), cmap='gray', vmin=0, vmax=255)
...
def find_valid_positions(possible_placements, obj_width, obj_height, y_top, y_bottom, image_shape):
    ...
    starty = random.randint(y_top, y_top+10)
    startx = random.randint(0, 10)
    step = random.randint(1, 10)

    # Iterate through all possible top-left positions
    for y in range(starty, min(y_bottom - obj_height + 1, img_height - obj_height + 1), step):
        for x in range(startx, img_width - obj_width + 1, step):
...
def rotate_cell(cell, rotation_angle_max=180):
    """
    h, w = new_arr.shape
    angle = random.randint(-rotation_angle, rotation_angle)
    M = cv2.getRotationMatrix2D((w // 2, h // 2), angle, 1.0)
    ...
    cell_rotated = scipy_ndimage_rotate(
        cell,
        angle=random.randint(-rotation_angle_max, rotation_angle_max),
        reshape=True
    )
    ...
    for split, ds, N_images in [
        ...
        for data_index in tqdm(range(len(plan))):
            ...
            for idx, (label, (cell, mask)) in enumerate(zip(labels, cells_at_index)):
                ...
                cell = rotate_cell(cell_recolored, 180)

                # Resize cell
                cell = cv2.resize(cell, (new_width, new_height), interpolation=cv2.INTER_AREA)
                ...
                valid_positions = find_valid_positions(possible_placements, new_width, new_height, 0, in_process_images_edge_size, image.shape)
